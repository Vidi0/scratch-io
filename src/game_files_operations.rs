...
pub fn add_part_extension(file: impl AsRef<Path>) -> Result<PathBuf, String> {
  let filename = file
    ...
    .ok_or_else(|| {
      format!(
        "Couldn't add .part extension to the file because it doesn't have a name!: {}",
...
pub fn is_folder_empty(folder: impl AsRef<Path>) -> Result<bool, String> {
  if folder.as_ref().is_dir() {
    ...
    Err(format!(
      "Error while cheching if folder is empty: \"{}\" is not a folder!",
...
pub async fn remove_folder_if_empty(folder: impl AsRef<Path>) -> Result<bool, String> {
  ...
  tokio::fs::remove_dir(&folder).await.map_err(|e| {
    format!(
...
pub fn make_executable(path: impl AsRef<Path>) -> Result<(), String> {
  #[cfg(unix)]
  {
    ...
    let metadata = std::fs::metadata(&path).map_err(|e| {
      format!(
        "Couldn't read file metadata of \"{}\": {e}",
        path.as_ref().to_string_lossy()
      )
    })?;
...
async fn move_folder_child(
  ...
  let mut child_entries = tokio::fs::read_dir(&last_root).await.map_err(|e| {
    format!(
      ...
      last_root.as_ref().to_string_lossy()
    )
  })?;
  ...
  while let Some(child) = child_entries.next_entry().await.map_err(|e| {
    ...
    if to.try_exists().map_err(|e| {
      ...
      tokio::fs::rename(&from, &temporal_name)
        .await
        .map_err(|e| {
          format!(
            "Couldn't move the item:\n  Source: \"{}\"\n  Destination: \"{}\"\n{e}",
            from.to_string_lossy(),
            temporal_name.to_string_lossy()
          )
        })?;
